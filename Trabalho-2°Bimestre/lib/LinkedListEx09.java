...
public class LinkedListEx09<T> {
    ...
    public void removeAll(T val) {
        ...
        while (current != null) {
            // Se encontrou o valor procurado
            if (current.data.equals(val)) {
                // caso 1: cabeça
                if (current == this.head) {
                    this.head = this.head.next;
                    current = this.head; // Avança para o novo Head
                }
                // caso 2: meio ou cauda
                else {
                    previous.next = current.next;
                    // se for ultimo, atualiza a cauda
